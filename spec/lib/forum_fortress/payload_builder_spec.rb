...
ForumFortressPayloadUserDouble = Struct.new(
  :username,
  :email,
  :created_at,
  :post_count,
...
RSpec.describe ForumFortress::PayloadBuilder do
  ...
  it "builds a topic payload with title, body, request IP, and user agent" do
    user = ForumFortressPayloadUserDouble.new(
      username: "new-user",
    ...
    manager = ForumFortressPayloadManagerDouble.new(
      user:,
      args: { title: "A title", raw: "A body", ip_address: "198.51.100.3", user_agent: "Test" },
    )
