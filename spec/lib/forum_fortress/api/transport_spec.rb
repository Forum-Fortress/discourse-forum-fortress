...
RSpec.describe ForumFortress::Api::Transport do
  ...
  it "posts JSON over HTTPS and decodes an object response" do
    request = stub_request(:post, "https://api.example/v1/check/reply")
      .with(
        headers: {
  ...
  it "rejects non-HTTPS endpoints" do
    expect {
      transport.get_json("http://api.example", "/health", timeout: 1)
    }.to raise_error(ForumFortress::Api::RequestError) { |error|
      expect(error.code).to eq("invalid_endpoint")
    }
  end
  ...
  it "reduces remote errors to a safe code" do
    ...
    expect {
      transport.get_json("https://api.example", "/health", timeout: 1)
    }.to raise_error(ForumFortress::Api::RequestError) { |error|
  ...
  it "maps a whole-request timeout to the established safe code" do
    ...
    expect {
      transport.get_json("https://api.example", "/health", timeout: 0.1)
    }.to raise_error(ForumFortress::Api::RequestError) { |error|
  ...
  it "rejects malformed and oversized responses" do
    ...
    expect {
      transport.get_json("https://api.example", "/malformed", timeout: 1)
    }.to raise_error(ForumFortress::Api::RequestError) { |error|
      expect(error.code).to eq("invalid_response")
    }
    expect {
      transport.get_json("https://api.example", "/oversized", timeout: 1)
    }.to raise_error(ForumFortress::Api::RequestError) { |error|
      expect(error.code).to eq("response_too_large")
    }
  end
end
