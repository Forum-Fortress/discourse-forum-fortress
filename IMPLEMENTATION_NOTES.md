...
- Generic first-time bootstrap has a separate 30-second bound because provisioning may traverse an edge and the control plane, and the generic endpoint cannot replay an API key after a lost first response. Normal anti-spam checks retain the short administrator-configured budget.
